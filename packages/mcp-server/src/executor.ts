...
function generateAgentThinking(agentType: string, task: any, goal: string, allTasks: any[], completedCount: number): string[] {
  const thoughts: Record<string, string[]> = {
    scout: [
      `Analyzing research goal: "${task.name}"`,
      `Breaking down research scope for: ${goal}`,
      `Identifying key information sources and data points`,
      `Checking ${task.dependencies?.length || 0} task dependencies`,
      'Synthesizing findings and preparing summary',
    ],
    coder: [
      `Planning implementation: "${task.name}"`,
      `Analyzing dependencies: ${task.dependencies?.join(', ') || 'none'}`,
      'Reviewing previous task outputs',
      'Designing solution architecture',
      'Writing production code',
    ],
    tester: [
      `Analyzing test requirements: "${task.name}"`,
      `Checking implementation from coder`,
      'Identifying edge cases',
      'Writing unit and integration tests',
      'Executing and validating test results',
    ],
    reviewer: [
      `Starting review: "${task.name}"`,
      `Checking code quality metrics`,
      'Evaluating security implications',
      'Providing improvement suggestions',
      'Finalizing review feedback',
    ],
    docs: [
      `Planning documentation: "${task.name}"`,
      `Gathering context from completed tasks`,
